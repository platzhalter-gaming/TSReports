...
public class Luck {
    ...
    public static void addPermission(UUID userUuid, Node node) {
        ...
        TSReports.getInstance().getLuckPerms().getUserManager().modifyUser(userUuid, user -> {
            // Add the permission
    ...
    public static CachedPermissionData getOfflineData(UUID uuid) {
        UserManager userManager = TSReports.getInstance().getLuckPerms().getUserManager();
        CompletableFuture<User> userFuture = userManager.loadUser(uuid);
        QueryOptions queryOptions = TSReports.getInstance().getLuckPerms().getContextManager().getQueryOptions(userFuture);
    ...
    public static CachedPermissionData getPermissionData(ProxiedPlayer player) {
        if (player.isConnected()) {
            User user = TSReports.getInstance().getLuckPerms().getPlayerAdapter(ProxiedPlayer.class).getUser(player);
            QueryOptions queryOptions = TSReports.getInstance().getLuckPerms().getContextManager().getQueryOptions(player);
